...
#if defined(__illumos__) && defined(V8_TARGET_ARCH_64_BIT)
  payload >>= kObjectPointerShift;
  // Unsigned types won't sign-extend on shift-right, but we need to with
  // illumos VA48 addressing.
  if ((payload & 0x800000000000ull) != 0)
	  payload |= 0xffff000000000000ull;
  return (payload | kHeapObjectTag);
#else
